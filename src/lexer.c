...
/*
 * Adds space for space for another
 * argument in `argv` array of `token`
 * of `type` `COMMAND`
 */
int
...
/*
 * For each argument in `argv` array of `COMMAND`
...
/*
 * Main interface for copying the argument from `string`
 * to `argv` of `COMMAND`
 */
int
...
tokenize_command (char *string, size_t *advance, Token *token)
{
  ...
  *advance = 1;
  ...
  while (1)
    {
      /* 
       * If any of the defined `Token_type` is found, create an 
      ...
      if (string[arg_len] == ' ' || string[arg_len] == '&'
          ...
          || string[arg_len] == '\0')
        {
          ...
          /* Move advance till the next argument (if any) */
          *advance += arg_len;
